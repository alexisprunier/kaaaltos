...
function CPB(url, max_per_page, url_cat, url_search) {
    ...
    this.getListTorrents = function (category, order, quality, limit) {
    ...
    this.getHTML =  function (url, page) {
        if (url.indexOf('?') === -1) {
            url += "/page-" + str(page);
        } else {
            url += "&page=" + str(page);
    ...
    this.search = function (url, quality, query) {
        url_search = url + query;
        return this._getRows()
    }

    this.getTorrentLink = function (url) {
    ...
    this.getLastIndexTitle = function () {
    ...
    this.getSeasonEpisode = function () {
