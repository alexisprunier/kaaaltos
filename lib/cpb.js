...
var getLastIndexTitle = function (tabTitle) {
    ...
    for (var i = 0; i < len; i++) {
        value = tabTitle[i];
        if (value === value.toUpperCase() && i > 0) {
            return i;
        }
    }
}
...
var getQuality = function (tabTitle, quality) {
    ...
    return tabTitle[indexQuality];

}
CPB.prototype.getRows = function (url, limit, page, list_quality, category) {
    var name, link, seeders, leechers, size, quality;
    var list = [];
    ...
    request(url, function (error, response, html) {
        if (!error && response.statusCode == 200) {
            ...
            $('.ligne0, .ligne1').each(function (i, element) {
                ...
                tor = new Torrent(cleanName, link, seeders, leechers, size, null, null, quality, category);
                list.push(tor);

            });
            console.log('tor', list);
...
CPB.prototype.search = function (quality, query) {
